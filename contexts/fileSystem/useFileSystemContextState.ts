    ...
    async (FS: EmscriptenFS, fsName?: string) =>
      new Promise<string>((resolve, reject) => {
        import("public/System/BrowserFS/extrafs.min.js").then((ExtraFS) => {
          ...
          Emscripten?.Create({ FS }, (error: unknown, newFs?: unknown) => {
            const emscriptenFS =
            ...
            try {
              rootFs?.mount?.(join("/", dbName), newFs as FileSystem);
            } catch {
    ...
    async (
      ...
      return new Promise((resolve, reject) => {
        HTTPRequest?.Create(
          ...
          (error: unknown, newFs?: unknown) => {
            if (error || !newFs) {
              reject(new Error("Error while mounting HTTPRequest FS."));
            } else {
              rootFs?.mount?.(mountPoint, newFs as FileSystem);
              resolve();
            }
          }
        );
        ...
        if (handle instanceof FileSystemDirectoryHandle) {
          import("public/System/BrowserFS/extrafs.min.js").then((ExtraFS) => {
            ...
            FileSystemAccess?.Create(
              ...
              rootFs?.mount?.(mappedPath, newFs as FileSystem);

...
const useWallpaper = (
  ...
  const loadFileWallpaper = useCallback(async () => {
    ...
    if (isSlideshow) {
      ...
      // Prefer a stable direct URL for built-in public wallpapers to avoid
      // creating a new blob URL on each render cycle (can cause flicker).
      if (wallpaperImage.startsWith("/Users/Public/")) {
        wallpaperUrl = `${window.location.origin}${wallpaperImage}`;
      } else {
